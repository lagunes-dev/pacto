...
const assertions = [
  ...
  [realtimePush.includes("row_number() over") && realtimePush.includes("partition by recipients.recipient_id") && realtimePush.includes("where recipient_rank = 1"), "Realtime backfill deduplicates each recipient before upsert"],
  [realtimePush.includes("order by (p.status <> 'ended') desc") && realtimePush.includes("p.updated_at desc nulls last") && realtimePush.includes("p.accepted_at desc nulls last") && realtimePush.includes("p.created_at desc") && realtimePush.includes("p.id desc"), "Realtime backfill uses authoritative revocation semantics and stable ordering"],
