...
## Browser and iPhone validation

Automated unit tests and the production build validate metadata, asset references, connectivity messaging, and safe-area declarations. Real installability, standalone launch, update prompts, and notched-device layout still require:

- a deployed HTTPS build,
- a supported desktop browser with PWA inspection tools, and
- a physical iPhone or iPad running Safari.

Those prerequisites are not available in the repository test environment, so this document does **not** claim browser or iPhone validation.
