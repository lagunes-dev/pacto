...
- `/partnership` exposes the development-only invite, redeem, reject, cancel, pause, and terminal end lifecycle.
- `/partnership/preferences` reads and updates only the authenticated owner's choices.
- `/partnership/support` permits typed, explicit request/acknowledge/close actions only while the partnership is active.
...
`VITE_DATA_ADAPTER=fixture` is development-only. Its users, habits, progress, partnership, preferences, and support requests live in memory and disappear after a reload. Explicit allow-list DTOs exclude private notes, habit details, summaries, email, and arbitrary metadata. This demonstrates frontend contract behavior but does not prove durable authentication, database persistence, authorization, or synchronization.
...
- Real Supabase/RLS, shared summaries, habit sharing, Realtime, Push, email/jobs, automatic alerts, offline queues, and deployment.
