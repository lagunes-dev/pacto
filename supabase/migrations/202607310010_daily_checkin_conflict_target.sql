...
-- Resolve entry_date through the unique constraint instead of the RETURNS TABLE
-- variable, preserving the owner/local-day idempotent upsert.
create or replace function public.save_daily_checkin(p_timezone text, p_craving_level smallint, p_habits jsonb)
...
begin
  ...
  on conflict on constraint daily_entries_user_id_entry_date_key do update
